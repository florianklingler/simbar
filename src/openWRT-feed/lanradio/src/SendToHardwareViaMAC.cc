...
SendToHardwareViaMAC::SendToHardwareViaMAC(string ownerModule,string ethernetDevice) {
	...
	if (getuid() != 0){
	    ...
	    std::cerr << "Root privileges are needed" << std::endl;
		exit(1);
	}
	...
	if (setsockopt(mSocket_VI,SOL_SOCKET ,SO_PRIORITY, &PRIORITY_VI,sizeof(PRIORITY_VI)) == -1){
	    ...
	    std::cerr << "Setsockop(priority) failed" << std::endl;
		exit(1);
	}
...
void SendToHardwareViaMAC::send(string* msg, int priority){
    ...
    // std::cout << "SendToHardwareViaMAC: packet size: " << packetsize << std::endl;
	if(send_to_socket != -1){
...
void SendToHardwareViaMAC::sendWLAN(string* msg, int priority, string* srcmac, string* dstmac, uint16_t* ether_type) {

    //std::cerr << "sending WLAN: " << std::endl;;
    ...
    //Destination mac
    memcpy(mEth_hdr.ether_dhost,(u_char *)ether_aton(dstmac->c_str()),ETHER_ADDR_LEN);

    //Source Mac
    memcpy(mEth_hdr.ether_shost,(u_char *)ether_aton(srcmac->c_str()),ETHER_ADDR_LEN);

    //ether type
    mEth_hdr.ether_type = htons(*ether_type);
    ...
    //copy payload to packet
    //std::cerr << "payload length: " << msg->size() << std::endl;
    memcpy(payload,msg->c_str(),msg->size());
